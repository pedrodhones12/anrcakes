...
if (canvas && stage && track) {
  ...
  let w=0,h=0,dpr=1,stageIndex=-1,time=0;
  ...
  function setStage(i){
    ...
    window.dispatchEvent(new CustomEvent('ane-stage-change',{detail:{stage:i}}));
  }
  ...
  function frame(){
    ...
    particles.forEach(drawParticle); drawNetwork(); drawCacao(stageIndex<0?0:stageIndex);
    canvas.dataset.threeReady='true'; requestAnimationFrame(frame);
  }

  // --- Controlled step-through navigation (wheel / keyboard / touch) ---
  let navLocked=false, navTimer=null;
  function lockNav(){ navLocked=true; clearTimeout(navTimer); navTimer=setTimeout(()=>{navLocked=false;},650); }
  ...
  function advance(dir){
    ...
    setStage(next); return true;
  }
  window.addEventListener('wheel',(e)=>{
    if(!isJourneyActive()) return;
    const dir = e.deltaY > 0 ? 1 : -1;
    if(dir > 0 && stageIndex >= 7){
      const bottom = track.getBoundingClientRect().top + window.scrollY + track.offsetHeight;
      window.scrollTo({ top: bottom, behavior: 'smooth' });
      return;
    }
    if(dir < 0 && stageIndex <= 0) return;
    e.preventDefault();
    if(navLocked) return;
    if(advance(dir)) lockNav();
  }, { passive: false });
  window.addEventListener('keydown',(e)=>{
    if(!isJourneyActive()) return;
    if(e.key==='ArrowDown'||e.key==='ArrowRight'){ e.preventDefault(); if(!navLocked&&advance(1)) lockNav(); }
    else if(e.key==='ArrowUp'||e.key==='ArrowLeft'){ e.preventDefault(); if(!navLocked&&advance(-1)) lockNav(); }
  });
  let touchStartY=null;
  stage.addEventListener('touchstart',(e)=>{ touchStartY=e.touches[0].clientY; },{passive:true});
  stage.addEventListener('touchend',(e)=>{
    if(touchStartY===null||!isJourneyActive()) return;
    const dy=touchStartY - e.changedTouches[0].clientY;
    if(Math.abs(dy)<30) return;
    const dir = dy > 0 ? 1 : -1;
    if(dir>0 && stageIndex>=7) return;
    if(dir<0 && stageIndex<=0) return;
    if(!navLocked && advance(dir)) lockNav();
    touchStartY=null;
  },{passive:true});
  // Allow the IA chat "Continuar" button to drive the visual stage
