...

{
  ...
  for(let i=0;i<6;i++){const leaf=add(g,new THREE.SphereGeometry(.8,16,12),(i%3-1)*.9,2+(i%2)*.25,(i%2)*.25);leaf.scale.set(1.5,.5,.8);leaf.material=material(colors.green);}
  const pod=add(g,new THREE.SphereGeometry(.78,28,20),.48,1,.25);pod.scale.set(.65,1.25,.65);pod.material=material(colors.red,.42);
}
{
  ...
  for(let i=-2;i<=2;i++){const p=add(g,new THREE.SphereGeometry(.25,12,8),i*.8,-.6);p.scale.y=1.6;p.material=material(colors.green);}
}
{
  const g=newGroup(); for(let i=0;i<3;i++){const drum=add(g,new THREE.CylinderGeometry(.65,.65,1.5,24),-1.45+i*1.45,0);drum.rotation.z=Math.PI/2;drum.material=material(i===1?colors.gold:colors.clay);} add(g,new THREE.BoxGeometry(5.2,.18,1.2),0,-.9).material=material(colors.brown); for(let i=0;i<8;i++){const bean=add(g,new THREE.SphereGeometry(.16,12,8),-2.1+i*.6,-.65);bean.scale.set(1,.55,.7);bean.material=material(0x6a351f);}
}
{
  const g=newGroup(); add(g,new THREE.BoxGeometry(5.4,2.5,2.8),0,0).material=material(0xd8c5b5); for(let i=0;i<3;i++) add(g,new THREE.CylinderGeometry(.35,.35,2.2,20),-1.7+i*1.7,2.25).material=material(colors.brown); add(g,new THREE.BoxGeometry(3.8,.65,.08),0,.4,1.43).material=material(0x8b6b5b);
}
{
...
{
  const g=newGroup(); for(let i=0;i<3;i++) add(g,new THREE.CylinderGeometry(1.45-i*.18,1.45-i*.18,.62,40),0,-.45+i*.62).material=material(i===1?0xf1d6c7:colors.cream); add(g,new THREE.TorusGeometry(.95,.16,14,40),0,1.42).material=material(colors.red); for(let i=0;i<6;i++) add(g,new THREE.SphereGeometry(.12,12,8),Math.cos(i*Math.PI/3)*.85,1.65,Math.sin(i*Math.PI/3)*.85).material=material(colors.red);
}
{
  const g=newGroup(); add(g,new THREE.BoxGeometry(5.2,.22,2.5),0,-.35).material=material(colors.brown); for(let i=0;i<4;i++) add(g,new THREE.SphereGeometry(.27,16,12),-1.7+i*1.15,.25,i%2?-.9:.9).material=material(i%2?colors.red:colors.clay); add(g,new THREE.CylinderGeometry(.8,.8,.08,40),0,.02).material=material(0xfffaf4);
}
{
  const g=newGroup(); add(g,new THREE.SphereGeometry(1.8,40,28),0,.35).material=material(colors.green); const ring=add(g,new THREE.TorusGeometry(2.3,.055,8,64),0,.35); ring.rotation.x=.45; ring.rotation.z=.2; ring.material=material(colors.red,.3,.35); add(g,new THREE.ConeGeometry(.16,.5,18),1,2.2,.1).material=material(colors.red);
}

const network=new THREE.Group(); world.add(network); network.position.set(0,.5,-1.8);
const pointPositions=[]; const pointArray=new Float32Array(42*3);
for(let i=0;i<42;i++){const a=Math.random()*Math.PI*2,r=2.2+Math.random()*2.2,p=new THREE.Vector3(Math.cos(a)*r,(Math.random()-.5)*2.5,Math.sin(a)*r*.45);pointPositions.push(p);pointArray[i*3]=p.x;pointArray[i*3+1]=p.y;pointArray[i*3+2]=p.z;}
const pointGeometry=new THREE.BufferGeometry(); pointGeometry.setAttribute('position',new THREE.BufferAttribute(pointArray,3)); network.add(new THREE.Points(pointGeometry,new THREE.PointsMaterial({color:colors.gold,size:.065,transparent:true,opacity:.6})));
const edges=[]; for(let i=0;i<pointPositions.length;i++) for(let j=i+1;j<pointPositions.length;j++) if(pointPositions[i].distanceTo(pointPositions[j])<1.5) edges.push(pointPositions[i].x,pointPositions[i].y,pointPositions[i].z,pointPositions[j].x,pointPositions[j].y,pointPositions[j].z);
const edgeGeometry=new THREE.BufferGeometry(); edgeGeometry.setAttribute('position',new THREE.Float32BufferAttribute(edges,3)); network.add(new THREE.LineSegments(edgeGeometry,new THREE.LineBasicMaterial({color:colors.red,transparent:true,opacity:.09})));
...
function update(){targetProgress=getProgress();progressValue+=(targetProgress-progressValue)*.12;const raw=progressValue*(stages.length-1);const index=Math.min(stages.length-1,Math.floor(raw+.00001));setStage(index);groups.forEach((g,i)=>{const distance=Math.abs(i-raw),opacity=THREE.MathUtils.clamp(1-distance*2.1,0,1);g.position.x=(i-raw)*.85;g.position.y=Math.sin((i-raw)*.7)*.12;g.scale.setScalar(.72+opacity*.34);g.traverse(obj=>{if(obj.isMesh)obj.material.opacity=opacity;});g.rotation.y+=i===index?.006:.001;});network.rotation.y+=.001;camera.position.x+=((raw-3.5)*.28-camera.position.x)*.045;camera.position.y+=(1.15+Math.sin(raw*.6)*.15-camera.position.y)*.045;camera.lookAt(0,.3,0);}
